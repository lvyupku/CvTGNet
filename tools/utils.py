...
#     label = ['Atelectasis', 'Cardiomegaly', 'Consolidation', 'Edema', 'Enlarged Cardiom.', 'Fracture', 'Lung Lesion', 'Lung Opacity', 'Effusion', 'Pleural Other', 'Pneumonia', 'Pneumothorax', 'Support Devices']
    ...
    for i in range(c):
        ...
        plt.plot(fpr, tpr, lw=lw, label=f'{label[i]}:{auc[i]:.3f}')
        #print('fpr', fpr, 'tpr', tpr, label[i])

    plt.xlabel('False Positive Rate')
    plt.ylabel('True Positive Rate')
#     plt.title(f'ChestX-Ray14, Mean={np.mean(auc):.3f}')
    plt.title(f'CheXpert, Mean={np.mean(auc):.3f}')
